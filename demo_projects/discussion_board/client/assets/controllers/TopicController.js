app.controller("TopicController", ['$scope', '$location', '$cookies', '$routeParams', 'topicsFactory', function($scope, $location, $cookies, $routeParams, topicsFactory) {
			...
			topicsFactory.single_topic($routeParams.id , function(status, topic) {
				if (status == false) {// the topic cannot be found using the id
					...
					$scope.updateTopic = {
								...
								}; // for the sake of textarea ng-model
				}
			});
	...
	$scope.update_topic = function(topic_id) {
		topicsFactory.update_topic(topic_id, $scope.updateTopic, function(status, response) {
			if (status == false) { // updated topic has failed the validation requirement
				// send back the errors
				$scope.updateErrors = response;
			}
			else { // update has been successfully completed. reload the page to see the updated topic!
				location.reload();
			}
		});
	...
	$scope.delete_topic = function(topic_id) {
		topicsFactory.delete_topic(topic_id);
		// after deleting request is sent to the factory, redirect user to the /dashbaord,
		// since the topic will be deleted.
		$location.url('/dashboard');
		location.reload();
	}
